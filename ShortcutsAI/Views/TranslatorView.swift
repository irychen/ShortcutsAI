...
struct TranslatorView: View {
  ...
  private func translateText() {
    ...
    var data = OpenAICompletionRequest(
      ...
      messages: [],
      temperature: OpenAIService.isSpecialModel(translateModel) ? 1 : Float(translateTemperature)
    )
    data.messages.append(
      OpenAIMessage(
        role: OpenAIService.isSpecialModel(translateModel) ? OpenAIRole.user : OpenAIRole.system,
        content: prompt
      ))
    data.messages.append(
      OpenAIMessage(
    ...
    do {
      loading = true
      try openAISvc.send(request: data)
      cancel = openAISvc.stopListening
      openAISvc.onDataReceived = { data in
        self.translateOutputText += data.text
        if data.isDone {
          self.loading = false
          DispatchQueue.main.async {
            try! HistoryService.shared.create(
              HistoryDto(
                name: "Translator", input: self.translateInputText, result: self.translateOutputText
              ))
          }
          return
        }
